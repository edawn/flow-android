    ...
    companion object {
        ...
        const val TRANSACTIONS_EDITS_TIME_SPAN_QUERY =
                "SELECT $EDITS_TABLE_NAME.*, $TRANSACTIONS_KEY_IS_REMOVED FROM $EDITS_TABLE_NAME INNER JOIN " +
                        ...
                        "$TRANSACTIONS_KEY_IS_REMOVED AND $EDITS_KEY_TRANSACTION_TIME >= ? AND $EDITS_KEY_TRANSACTION_TIME < ? GROUP BY $EDITS_KEY_TRANSACTION) " +
                        "editsmax ON $EDITS_TABLE_NAME.$EDITS_KEY_ID = editsmax.$EDITS_KEY_ID ORDER BY $EDITS_KEY_TRANSACTION_TIME"
